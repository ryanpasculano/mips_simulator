...
public:
	 ...
	 */
	virtual uint32_t getTarget(uint32_t PC, int is_branch, int branch_cond) = 0;
	/*
	 * It is called after each branch is resolved to update
	 * the branch predictor metadata
	 */
	virtual void update(uint32_t PC, bool taken, uint32_t target, int is_branch, int branch_cond) = 0;
};
